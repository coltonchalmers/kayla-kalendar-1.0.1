...
export function useProposalLinks() {
  ...
  const createProposal = useCallback(async (input: {
    ...
    const { data: link, error: linkError } = await supabase
      ...
      .insert({
        ...
        internal_notes: linkData.internal_notes || null,
        notes_to_client: linkData.notes_to_client || null,
        allow_full_availability: linkData.allow_full_availability ?? true,
      })
      .select()
      .single();
  ...
  const updateProposal = useCallback(async (id: string, updates: {
    client_name?: string;
    client_email?: string;
    label?: string | null;
    meeting_type_id?: string | null;
